...
pub fn format_source(source: &str, path: Option<&Path>) -> Result<String, String> {
    let invocations =
    ...
    for invocation in invocations {
        ...
        let nested = format_source(body, path)?;
...
fn format_if(nodes: &[Node], cursor: &mut usize, indent: usize) -> Result<String, String> {
    *cursor += 1;
    ...
    let condition = format_generic(&nodes[*cursor..branch], indent + 3);
...
fn format_for(nodes: &[Node], cursor: &mut usize, indent: usize) -> Result<String, String> {
    *cursor += 1;
    ...
    let header = format_generic(&nodes[*cursor..body_at], indent + 4);
